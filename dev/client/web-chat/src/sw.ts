...
function postRouteCMatrixResumeMessage(client: Client, targetUrl: string, sameUrl: boolean): void {
  client.postMessage({
    type: ROUTEC_MATRIX_RECOVERY_SW_MESSAGE,
    reason: 'notificationclick',
    targetUrl,
    sameUrl,
  ...
  event.waitUntil(
    (async () => {
      ...
      for (const client of windowClients) {
        ...
        const sameUrlBeforeNavigation = client.url === targetUrl;
        ...
        postRouteCMatrixResumeMessage(targetClient, targetUrl, sameUrlBeforeNavigation);
      ...
      if (self.clients.openWindow) {
        ...
        if (opened) {
          postRouteCMatrixResumeMessage(opened, targetUrl, false);
        }
        return opened;
      }
      return undefined;
