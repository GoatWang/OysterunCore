...
import {
  ...
  createOysterunRouteCOptimisticClientRequestId,
...
async function getRouteCStagedMediaInfo(fileItem: TUploadItem): Promise<Record<string, unknown>> {
  const { originalFile } = fileItem;
  const info: Record<string, unknown> = {
    mimetype: getRouteCMediaMimeType(originalFile),
  ...
  try {
    ...
    return {
      ...info,
      ...getImageInfo(imageElement, originalFile),
    };
  } catch (err) {
...
async function uploadRouteCStagedMedia({
  ...
  const mimetype = getRouteCMediaMimeType(fileItem.file);
  const uploaded = (await mx.uploadContent(fileItem.file, {
    name: filename,
    type: mimetype,
  ...
  const info = await getRouteCStagedMediaInfo(fileItem);
  ...
  return {
    ...
    byte_size: uploaded.byte_size ?? fileItem.file.size,
...
export const RoomInput = forwardRef<HTMLDivElement, RoomInputProps>(
  ({ editor, fileDropContainerRef, roomId, room, routeCRespondingState }, ref) => {
    ...
    const submit = useCallback(async () => {
      ...
      const optimisticClientRequestId = hasOysterunHostSessionRoute()
        ? createOysterunRouteCOptimisticClientRequestId()
        : undefined;
    ...
    const handleRouteCPrimaryAction = useCallback(() => {
      ...
      void submit();
    }, [roomId, routeCPrimaryAction, sendTypingStatus, submit]);
    ...
    const handleKeyUp: KeyboardEventHandler = useCallback(
      ...
      [editor, sendTypingStatus, hideActivity, refreshAutocompleteQuery]
    );
