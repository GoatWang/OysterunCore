...
const OYSTERUN_PIN_MENU_MIN_VISIBLE_PENDING_MS = 500;
type MessagePinAction = 'pin' | 'unpin';
...
function getCurrentRoomPinnedEventIds(room: Room, fallbackPinnedEvents: string[]): string[] {
  ...
  return Array.isArray(content?.pinned) ? content.pinned : fallbackPinnedEvents;
}

function buildMessagePinStateContent(
  room: Room,
  fallbackPinnedEvents: string[],
  eventId: string,
  action: MessagePinAction
): RoomPinnedEventsEventContent {
  const currentPinnedEvents = getCurrentRoomPinnedEventIds(room, fallbackPinnedEvents);
  if (action === 'unpin') {
    return { pinned: currentPinnedEvents.filter((id) => id !== eventId) };
  }
  if (currentPinnedEvents.includes(eventId)) {
    return { pinned: currentPinnedEvents };
  }
  return { pinned: [...currentPinnedEvents, eventId] };
}

function waitForOysterunPinMenuMinimumVisiblePending(): Promise<void> {
  return new Promise((resolve) => {
    globalThis.setTimeout(resolve, OYSTERUN_PIN_MENU_MIN_VISIBLE_PENDING_MS);
  });
}
...
>(({ room, mEvent, onClose, ...props }, ref) => {
  ...
  const [pendingPinAction, setPendingPinAction] = useState<MessagePinAction>();

  const [pinState, sendPinState] = useAsyncCallback(
    useCallback(
      async (eventId: string, action: MessagePinAction) => {
        const pinContent = buildMessagePinStateContent(room, pinnedEvents, eventId, action);
        await Promise.all([
          mx.sendStateEvent(room.roomId, StateEvent.RoomPinnedEvents as any, pinContent),
          waitForOysterunPinMenuMinimumVisiblePending(),
        ]);
      },
      [mx, room, pinnedEvents]
    )
  );

  const completePendingPin = () => {
    pendingRef.current = false;
    setPendingPinAction(undefined);
  };

  const handlePin = () => {
    const eventId = mEvent.getId();
    if (!eventId || pendingRef.current || pinState.status === AsyncStatus.Loading) return;
    const action: MessagePinAction = isPinned ? 'unpin' : 'pin';
    pendingRef.current = true;
    setPendingPinAction(action);
    void sendPinState(eventId, action)
      .then(() => {
        completePendingPin();
        onClose?.();
      })
      .catch((err: unknown) => {
        console.error('[oysterun-routec] Message pin state update failed', {
          roomId: room.roomId,
          eventId,
          action,
          err,
  ...
  const pinPending = pendingPinAction !== undefined || pinState.status === AsyncStatus.Loading;
  const pinFailed = pinState.status === AsyncStatus.Error;
  const pinAction: MessagePinAction = pendingPinAction ?? (isPinned ? 'unpin' : 'pin');
  const pinLabel = (() => {
    if (pinPending) return pinAction === 'pin' ? 'Pinning...' : 'Unpinning...';
    if (pinFailed) return pinAction === 'pin' ? 'Retry Pin Message' : 'Retry Unpin Message';
    return pinAction === 'pin' ? 'Pin Message' : 'Unpin Message';
  })();
  ...
  return (
    <>
      <MenuItem
        ...
        radii="300"
        onClick={pinPending ? undefined : handlePin}
        aria-disabled={pinPending}
        data-oysterun-message-operation={pinAction}
        data-oysterun-pin-action={pinAction}
        data-oysterun-pin-pending={String(pinPending)}
        data-oysterun-pin-duplicate-clicks-suppressed={String(pinPending)}
        data-oysterun-pin-menu-retained-while-pending="true"
        data-oysterun-pin-min-visible-pending-ms={OYSTERUN_PIN_MENU_MIN_VISIBLE_PENDING_MS}
        data-oysterun-pin-payload-source="current_room_state_at_action_time"
        data-oysterun-pin-failure-retryable={String(pinFailed)}
        {...props}
        ref={ref}
      >
        <Text className={css.MessageMenuItemText} as="span" size="T300" truncate>
      ...
      {pinFailed && (
        <Box
          ...
          data-oysterun-pin-failure="local_retryable"
        >
          <Text style={{ color: color.Critical.Main }} size="T200">
            {pinAction === 'pin' ? 'Pin failed.' : 'Unpin failed.'} Try again.
          </Text>
        </Box>
      )}
    </>
