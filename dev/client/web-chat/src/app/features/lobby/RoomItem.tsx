...
export const RoomItemCard = as<'div', RoomItemCardProps>(
  (
    ...
    const routecHostSessionId = getOysterunHostSessionId();
    const routecBoundRoomId = getOysterunMatrixRoomId();
    const routecBoundRoom = Boolean(
      routecHostSessionId
        && routecBoundRoomId
        && routecBoundRoomId === roomId
        && joined
    );
                  ...
                  options={
                    joined ? (
                      <Box shrink="No" gap="100" alignItems="Center">
                        <Chip
                          data-testid={routecBoundRoom ? 'oysterun-routec-room-row' : undefined}
                          data-oysterun-clean-session-testid={routecBoundRoom ? 'oysterun-clean-session-room-row' : undefined}
                          data-room-id={roomId}
                          data-oysterun-routec-room-entry-contract={
                            routecBoundRoom ? OYSTERUN_ROUTEC_ROOM_ENTRY_CONTRACT : undefined
                          }
                          data-oysterun-clean-session-room-entry-contract={
                            routecBoundRoom ? OYSTERUN_ROUTEC_ROOM_ENTRY_CONTRACT : undefined
                          }
                          data-oysterun-routec-room-row={
                            routecBoundRoom ? 'bound-room-actionable' : undefined
                          }
                          data-oysterun-clean-session-room-row={
                            routecBoundRoom ? 'bound-room-actionable' : undefined
                          }
                          data-oysterun-routec-room-entry-state={
                            routecBoundRoom ? 'bound_room_row_actionable' : undefined
                          }
                          data-oysterun-clean-session-room-entry-state={
                            routecBoundRoom ? 'bound_room_row_actionable' : undefined
                          }
                          data-oysterun-routec-room-entry-ready={
                            routecBoundRoom ? 'true' : undefined
                          }
                          data-oysterun-clean-session-room-entry-ready={
                            routecBoundRoom ? 'true' : undefined
                          }
                          data-oysterun-routec-bound-room-id={
                            routecBoundRoom ? routecBoundRoomId : undefined
                          }
                          data-oysterun-clean-session-bound-room-id={
                            routecBoundRoom ? routecBoundRoomId : undefined
                          }
                          data-oysterun-host-session-id={
                            routecBoundRoom ? routecHostSessionId : undefined
                          }
                          data-oysterun-room-id={routecBoundRoom ? routecBoundRoomId : undefined}
                          data-oysterun-routec-room-row-action={
                            routecBoundRoom ? 'open-bound-room' : undefined
                          }
                          data-oysterun-clean-session-room-row-action={
                            routecBoundRoom ? 'open-bound-room' : undefined
                          }
                          data-oysterun-routec-room-row-identity={
                            routecBoundRoom ? 'host_scoped_bootstrap_matrix_room_id' : undefined
                          }
                          data-oysterun-clean-session-room-row-identity={
                            routecBoundRoom ? 'host_scoped_bootstrap_matrix_room_id' : undefined
                          }
                          data-oysterun-routec-stale-text-selector-required={
                            routecBoundRoom ? 'false' : undefined
                          }
                          data-oysterun-clean-session-stale-text-selector-required={
                            routecBoundRoom ? 'false' : undefined
                          }
                          onClick={onOpen}
                          variant="Secondary"
