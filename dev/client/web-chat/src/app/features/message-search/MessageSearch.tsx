    ...
    (roomId: string, eventId: string) => {
      if (
        requestOysterunActiveRoomTimelineFocus({
          roomId,
          eventId,
          source: 'message_search_open',
        })
      ) {
        return;
      }
      const routeCChatFocusPath = getOysterunHostSessionChatFocusPath(eventId);
      if (routeCChatFocusPath) {
        navigate(routeCChatFocusPath);
        return;
      }
  ...
  const handleSelectedRoomsChange = (selectedRooms?: string[]) => {
    updateSearchParams((params) => {
      ...
      if (selectedRooms && selectedRooms.length > 0) {
        params.set('rooms', encodeSearchParamValueArray(selectedRooms));
      }
    });
  };
