...
const appendMatrixMediaAccessToken = (mx: MatrixClient, httpUrl: string): string => {
  try {
    const parsed = new URL(
      httpUrl,
      typeof window !== 'undefined' ? window.location.origin : 'http://oysterun.local'
    );
    if (!parsed.pathname.startsWith('/_matrix/media/')) return httpUrl;
    if (parsed.searchParams.has('access_token')) return httpUrl;

    const accessToken = mx.getAccessToken();
    if (!accessToken) return httpUrl;
...
export const mxcUrlToHttp = (
  ...
  return httpUrl ? appendMatrixMediaAccessToken(mx, httpUrl) : null;
};
