...
const getClientConfig = async (): Promise<ClientConfig> => {
  ...
  const payload = (await config.json()) as ClientConfig;
  return normalizeOysterunHostOriginConfig(payload);
};

const OYSTERUN_HOST_ORIGIN_SENTINEL = 'oysterun-host-origin';

function normalizeOysterunHostOriginConfig(config: ClientConfig): ClientConfig {
  const homeserverList = config.homeserverList?.map((server) =>
    server === OYSTERUN_HOST_ORIGIN_SENTINEL ? window.location.origin : server
  );
  return {
    ...config,
    homeserverList,
