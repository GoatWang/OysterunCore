...
export const OYSTERUN_P185_STALE_AUTOCOMPLETE_RANGE_GUARD =
  'p185-stale-autocomplete-range-guard';
...
export const isAutocompleteRangeCurrent = (
  editor: Editor,
  selectRange: BaseRange,
  expectedPrefix: string,
): boolean => {
  try {
    const currentText = Editor.string(editor, selectRange);
    if (!currentText.startsWith(expectedPrefix)) return false;
    Editor.range(editor, selectRange.anchor, selectRange.focus);
    return true;
  } catch {
    return false;
  }
...
const getActiveComposerTokenStart = (text: string): number | undefined => {
...
export const getPrevAutocompleteRange = (editor: Editor): BaseRange | undefined => {
  const { selection } = editor;
  if (!selection || !Range.isCollapsed(selection)) return undefined;
  const [cursorPoint] = Range.edges(selection);
  const blockPath = cursorPoint.path.slice(0, 1);
  ...
  const text = positions
    ...
    .map((point, index) =>
      Editor.string(editor, {
        anchor: positions[index] ?? startPoint,
        focus: point,
      }),
    )
    .join('');
  const tokenStart = getActiveComposerTokenStart(text);
  if (tokenStart === undefined) return undefined;
  const tokenStartPoint = positions[tokenStart];
  return tokenStartPoint && Editor.range(editor, tokenStartPoint, cursorPoint);
};
