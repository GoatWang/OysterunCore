...
export function setFallbackSession(
  ...
  if (!accessToken.startsWith('oysterun_facade_')) {
    throw new Error('Route C forbids storing raw long-lived Synapse tokens in browser storage.');
  }
  if (baseUrl !== window.location.origin) {
    throw new Error('Route C Matrix baseUrl must be the Host origin.');
  }
...
export const getFallbackSession = (): Session | undefined => {
  ...
  if (baseUrl && userId && deviceId && accessToken) {
    if (baseUrl !== window.location.origin || !accessToken.startsWith('oysterun_facade_')) {
