import {
  Direction,
  EventTimeline,
} from 'matrix-js-sdk/lib/models/event-timeline';
import type { IContextResponse } from 'matrix-js-sdk/lib/@types/requests';
...
import type { RoomServerAclEventContent } from 'matrix-js-sdk/lib/@types/state_events';
...
import { getOysterunHostSessionRouteSearch } from '../../oysterun/OysterunHostClient';
...
const ROUTE_C_APPROVED_COMMANDS = [] as const;

export const useCommands = (mx: MatrixClient, room: Room): CommandRecord => {
  const { navigateRoom } = useRoomNavigate();

  const commands: Record<Command, CommandContent> = useMemo(
    () => ({
      [Command.Me]: {
        name: Command.Me,
      ...
      [Command.Acl]: {
        ...
        exe: async (payload) => {
          ...
          )?.getContent() as RoomServerAclEventContent | undefined;
  ...
  const routeCCommands: CommandRecord = useMemo(() => {
    const filteredCommands: CommandRecord = {};
    ROUTE_C_APPROVED_COMMANDS.forEach((command) => {
      filteredCommands[command] = commands[command];
    });
    return filteredCommands;
  }, [commands]);

  if (getOysterunHostSessionRouteSearch()) {
    return routeCCommands;
  }
